...
module.exports = {
  fetch: function(task, callback){
    fetch_tag(task.stamp, task.page, function(page_count){
      if (page_count > 1) {
        for (var i = 2; i <= page_count; i++) {
          queue.push({ tag: task.tag, page: i })
        }
      }
      callback()
    })
...
function fetch_tag (tag, page, callback) {
  ...
  console.log("fetch stamp", tag, "page", page)
